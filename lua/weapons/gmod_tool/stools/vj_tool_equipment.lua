...
if CLIENT then
	local function ControlPanel(panel)
		...
		panel:Help("#tool.vj_tool_equipment.menu.label")
		...
		local wepText = vgui.Create("DTextEntry")
			...
			wepText:SetText(language.GetPhrase("#tool.vj_tool_equipment.menu.selected") .. ": " .. equipName .. " [" .. equipClass .. "]")
			function wepText:OnGetFocus()
				...
				local wepSelList = vgui.Create("DListView")
					...
					wepSelList:AddColumn("#tool.vj_tool_equipment.menu.header.name")
					wepSelList:AddColumn("#tool.vj_tool_equipment.menu.header.class")
					...
					function wepSelList:DoDoubleClick(lineID, line)
						...
						wepSelFrame:Close()
						wepText:SetText(language.GetPhrase("#tool.vj_tool_equipment.menu.selected") .. ": " .. language.GetPhrase(line:GetValue(1)) .. " [" .. line:GetValue(2) .. "]")
					end
				for _, v in pairs(list.Get("NPCUsableWeapons")) do
