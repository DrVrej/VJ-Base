...
-- ENT.DecalTbl_OnCollideDecals = {"Scorch"} -- Decals that paint when the projectile collides with something | It picks a random one from this table
...
function ENT:CustomOnInitialize()
	...
	util.SpriteTrail(self,0,Color(255,255,255,255),true,15,0,0.1,1 /(6 +0) *0.5,"sprites/combineball_trail_black_1.vmt")
	...
	local hookName = "VJ_CombineBall_" .. self:EntIndex() .. "_Dropped"
...
function ENT:OnBounce(data, phys)
	...
	for _,v in pairs(ents.FindInSphere(self:GetPos(),1024)) do
		...
		if !self:GetOwner():DoRelationshipCheck(v) then continue end
	...
	if IsValid(target) then
		...
		local norm = (targetPos -self:GetPos()):GetNormalized()
...
function ENT:CustomOnPhysicsCollide(data, phys)
	...
	if IsValid(self:GetOwner()) then
		if (VJ_IsProp(hitEnt)) or (self:GetOwner():DoRelationshipCheck(hitEnt) && (hitEnt != self:GetOwner())) then
...
function ENT:CustomOnDoDamage_Direct(data, phys, hitEnt)
	VJ_CreateSound(data.HitEntity,VJ_PICK({"weapons/physcannon/energy_disintegrate4.wav","weapons/physcannon/energy_disintegrate5.wav"}),80)
end
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:DeathEffects(data, phys)
	effects.BeamRingPoint(self:GetPos(),0.2,12,1024,64,0,Color(255,255,225,32),{material="sprites/lgtning.vmt",framerate=2,flags=0,speed=0,delay=0,spread=0})
	effects.BeamRingPoint(self:GetPos(),0.5,12,1024,64,0,Color(255,255,225,64),{material="sprites/lgtning.vmt",framerate=2,flags=0,speed=0,delay=0,spread=0})
	...
	util.VJ_SphereDamage(self,self,self:GetPos(),400,25,bit.bor(DMG_SONIC,DMG_BLAST),true,true,{DisableVisibilityCheck=true,Force=80})
