...
ENT.Weapon_NoSpawnMenu = false -- If enabled, it will ignore the weapon override setting from the spawn menu
		...
		if IsValid(self) then
			...
			if self.DisableWeapons then
				...
				if IsValid(wep) then
					...
					if IsValid(self:GetCreator()) && self.AllowPrintingInChat && !wep.IsVJBaseWeapon then
					...
					self:UpdateAnimationTranslations()
					if IsValid(self:GetCreator()) && self.AllowPrintingInChat && !self.Weapon_NoSpawnMenu then
...
function ENT:DoChangeMovementType(movType)
	if movType then
		...
		if movType == VJ_MOVETYPE_GROUND then
			...
			if (VJ.AnimExists(self, ACT_JUMP) && GetConVar("vj_npc_human_canjump"):GetInt() == 1) or self.UsePlayerModelMovement then self:CapabilitiesAdd(CAP_MOVE_JUMP) end
...
function ENT:TranslateActivity(act)
	...
	if act == ACT_IDLE then
		if self.NoWeapon_UseScaredBehavior_Active then
	...
	for _, ent in ipairs(ents.FindInSphere(self:GetPos(), self.DangerDetectionDistance)) do
		if (ent.VJTag_ID_Danger or ent.VJTag_ID_Grenade) && self:Visible(ent) then
			...
			if !(IsValid(vOwner) && vOwner.IsVJBaseSNPC && ((self:GetClass() == vOwner:GetClass()) or (self:Disposition(vOwner) == D_LI))) then
				...
				if self.CanThrowBackDetectedGrenades && self.HasGrenadeAttack && ent.VJTag_IsPickupable && !ent.VJTag_IsPickedUp && ent:GetVelocity():Length() < 400 && self:FindNearestDistance(ent) < 100 && self:GrenadeAttack(ent, true) then
							...
							if inCover then
								...
								elseif curTime > self.NextMoveOnGunCoveredT && ((inCoverTrace.HitPos:Distance(myPos) > 150 && !inCoverEntLiving) or (wepInCover && !wepInCoverEnt.VJTag_IsLiving)) then
									...
									if IsValid(inCoverEnt) then
										nearestPos, nearestEntPos = self:FindNearestPositions(inCoverEnt)
							...
							-- Ranged weapons
								...
								elseif self:GetActivity() != self.CurrentWeaponAnimation && self:GetActivity() != ACT_TRANSITION then
									...
									if !hasAmmo then
										...
										if self.CanCrouchOnWeaponAttack && !inCover && !wepInCover && self.LatestEnemyDistance > 500 && VJ.AnimExists(self, anim_crouch) && ((math.random(1, self.CanCrouchOnWeaponAttackChance) == 1) or (curTime <= self.Weapon_DoingCrouchAttackT)) && !self:DoCoverTrace(wep:GetBulletPos() + self:GetUp()*-18, enePos_Eye, true) then
