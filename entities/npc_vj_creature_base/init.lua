...
ENT.PushProps = true -- True = Pushes props (The ones in the code only!)
ENT.AttackProps = true -- True = Destroys props (The ones in the code only!)
...
ENT.MeleeAttackAnimationFaceEnemy_IsPropAttack = false
...
function ENT:Initialize()
	...
	if self.VJ_IsAerial == false then self:VJ_SetSchedule(SCHED_FALL_TO_GROUND) end
...
-- Add as many as you want --
	...
	if self.VJ_IsStationary == false && self.VJ_IsAerial == false then self:CapabilitiesAdd(bit.bor(CAP_MOVE_GROUND)) end
...
ENT.VJ_IsAerial = false -- Is this a aerial (flying) SNPC?
...
function ENT:AerialMove_ChaseEnemy(ShouldPlayAnim)
	...
	if ShouldPlayAnim == true && self:GetSequence() != self.AerialCurrentAnim /*&& self:GetActivity() == ACT_IDLE*/ && CurTime() > self.NextFlyAnim then
	...
	local ildeanimid = self:LookupSequence(ideanimrand)
		...
		//self:VJ_ACT_PLAYACTIVITY(ideanimrand,false,0,false,0,true)
...
function ENT:DoChaseAnimation(OverrideChasing)
	...
	if self.VJ_IsAerial == true then self:AerialMove_ChaseEnemy(true) else self:VJ_SetSchedule(VJ_PICKRANDOMTABLE(self.ChaseSchedule)) end
...
if self:GetEnemy() != nil then
	if self.VJ_IsAerial == true then self:AerialMove_ChaseEnemy() end
...
if self:GetEnemy() != nil then
	if self.VJ_IsBeingControlled == false then
		...
		if self.MeleeAttackAnimationFaceEnemy == true && self.MeleeAttackAnimationFaceEnemy_IsPropAttack == false && self.Dead == false && timer.Exists("timer_melee_start"..self.Entity:EntIndex()) && timer.TimeLeft("timer_melee_start"..self.Entity:EntIndex()) > 0 then self:FaceCertainEntity(self:GetEnemy(),true) end
...
if self.HasMeleeAttack == true then
	...
	if ispropattack == true then self.MeleeAttackAnimationFaceEnemy_IsPropAttack = true else self.MeleeAttackAnimationFaceEnemy_IsPropAttack = false end
	...
	for tk, tv in ipairs(self.MeleeAttackExtraTimers) do
		self:DoAddTimers("timer_melee_start_"..self:GetClass().."_"..math.random(1,999999),tv,1,"MeleeAttack")
	end
...
if self.HasRangeAttack == true then
	...
	if self.DisableRangeAttackAnimation == false then
		...
		self:VJ_ACT_PLAYACTIVITY(self.CurrentAttackAnimation,false,0,false,self.RangeAttackAnimationDelay,false,self.CurrentAttackAnimationDuration)
	end
	...
	for tk, tv in ipairs(self.RangeAttackExtraTimers) do
		self:DoAddTimers("timer_range_start_"..self:GetClass().."_"..math.random(1,999999),tv,1,"RangeAttack")
	end
...
if self.HasLeapAttack == true then
	...
	if self.DisableLeapAttackAnimation == false then
		...
		self:VJ_ACT_PLAYACTIVITY(self.CurrentAttackAnimation,false,0,false,self.LeapAttackAnimationDelay,false,self.CurrentAttackAnimationDuration)
	end
	...
	for tk, tv in ipairs(self.LeapAttackExtraTimers) do
		self:DoAddTimers("timer_leap_start_"..self:GetClass().."_"..math.random(1,999999),tv,1,"LeapAttack")
	end
	...
	if self.VJ_IsAerial == true && self:GetVelocity():Length() > 0 then self:SetLocalVelocity(Vector(0,0,0)) end
	//self:StopAttacks()
	//self:SelectSchedule()
 end
...
--------------------------------------------------------------------------------------------------------------------------------------------
function ENT:DoAddTimers(vName,vTime,vReps,vFunction)
...
function ENT:CanDoCertainAttack(AttackName)
	...
	if AttackName == "MeleeAttack" then
		if self.IsAbleToMeleeAttack == true && self.MeleeAttacking == false && self.LeapAttacking == false && self.RangeAttacking == false && self.vACT_StopAttacks == false && self.Flinching == false && self.VJ_IsBeingControlled == false /*&& self.VJ_PlayingSequence == false*/ then
		return true
		end
	end
	...
	if AttackName == "RangeAttack" then
		...
		if self.IsAbleToRangeAttack == true && (self:GetEnemy():GetPos():Distance(self:GetPos()) < self.RangeDistance) && (self:GetEnemy():GetPos():Distance(self:GetPos()) > self.RangeToMeleeDistance) && self:GetEnemy():Visible(self) && self.RangeAttacking == false && self.MeleeAttacking == false && self.vACT_StopAttacks == false && self.Flinching == false && self.VJ_IsBeingControlled == false /*&& self.VJ_PlayingSequence == false*/ then
		return true
		end
	end

	if AttackName == "LeapAttack" then
		if self.IsAbleToLeapAttack == true && self:IsOnGround() && self:GetPos():Distance(self:GetEnemy():GetPos()) < self.LeapDistance && (self:GetPos():Distance(self:GetEnemy():GetPos()) > self.LeapToMeleeDistance) && self:GetEnemy():Visible(self) && self.RangeAttacking == false && self.LeapAttacking == false && self.vACT_StopAttacks == false && self.Flinching == false && self.VJ_IsBeingControlled == false /*&& self.VJ_PlayingSequence == false*/ then
		return true
		end
	end
	return false
...
function ENT:PushOrAttackPropsCode(IsSingleValue,CustomMeleeDistance)
	local IsSingleValue = IsSingleValue or 0
	...
	if IsSingleValue == 0 then
		valuestoattack = ents.FindInSphere(self:GetPos(),self.MeleeAttackDistance *5)
	else
		valuestoattack = {IsSingleValue}
	end
	for _,v in pairs(valuestoattack) do
	local phys = v:GetPhysicsObject()
	local CustomMeleeDistance = CustomMeleeDistance or self.MeleeAttackDistance *2
	...
	if self:VJ_GetNearestPointToEntityDistance(v) < CustomMeleeDistance && self:DoPropVisibiltyCheckForPushAttackProps(v) /*&& self:Visible(v)*/ && (self:GetForward():Dot((v:GetPos() -self:GetPos()):GetNormalized()) > math.cos(math.rad(self.SightAngle / 1.5))) && phys:IsValid() && phys != nil && phys != NULL && v:GetCollisionGroup() != COLLISION_GROUP_DEBRIS && v:GetCollisionGroup() != COLLISION_GROUP_DEBRIS_TRIGGER && v:GetCollisionGroup() != COLLISION_GROUP_DISSOLVING && v:GetCollisionGroup() != COLLISION_GROUP_IN_VEHICLE then
...
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:MeleeAttackCode(IsPropAttack,AttackPosition,CustomEnt)
	...
	local AttackPosition = AttackPosition or self.MeleeAttackDamageDistance
	if IsPropAttack == true then AttackPosition = (self.MeleeAttackDamageDistance *2)/* + 50*/ end
	if self.VJ_IsBeingControlled == false && self.MeleeAttackAnimationFaceEnemy == true && self.MeleeAttackAnimationFaceEnemy_IsPropAttack == false then self:FaceCertainEntity(MyEnemy,true) end
	self.MeleeAttacking = true
	self:CustomOnMeleeAttack_BeforeChecks()
	if self.DisableDefaultMeleeAttackCode == true then return end
	local attackthev = ents.FindInSphere(self:GetPos() + self:GetForward(), AttackPosition)
	...
	if (self:GetForward():Dot((v:GetPos() -self:GetPos()):GetNormalized()) > math.cos(math.rad(self.MeleeAttackDamageAngleRadius))) then
		//if IsPropAttack == true && self:GetPos():Distance(v:GetPos()) <= AttackPosition /2 && v:GetClass() != "prop_physics" && v:GetClass() != "func_breakable_surf" && v:GetClass() != "func_breakable" then continue end
		...
		if v:GetClass() == "prop_physics" then
			...
			if self:PushOrAttackPropsCode(v,AttackPosition) then
			HasHitGoodProp = true
		...
		if self.DisableDefaultMeleeAttackDamageCode == false then
			...
			if GetConVarNumber("vj_npc_dif_normal") == 1 then doactualdmg:SetDamage(self.MeleeAttackDamage) end -- Normal
			if GetConVarNumber("vj_npc_dif_easy") == 1 then doactualdmg:SetDamage(self.MeleeAttackDamage/2) end -- Easy
			if GetConVarNumber("vj_npc_dif_hard") == 1 then doactualdmg:SetDamage(self.MeleeAttackDamage*1.5) end -- Hard
			if GetConVarNumber("vj_npc_dif_hellonearth") == 1 then doactualdmg:SetDamage(self.MeleeAttackDamage*2.5) end  -- Hell On Earth
	...
	timer.Create( "timer_melee_finished"..self.Entity:EntIndex(), self.NextAnyAttackTime_Melee, 1, function()
	 ...
	 timer.Create( "timer_range_finished_abletomelee"..self.Entity:EntIndex(), self.NextMeleeAttackTime, 1, function()
...
function ENT:RangeAttackCode()
	...
	if self.DisableDefaultRangeAttackCode == false then
		...
		rangeprojectile:SetAngles(self:GetAngles())
	...
	timer.Create( "timer_range_finished"..self.Entity:EntIndex(), self.NextAnyAttackTime_Range, 1, function()
		self:StopAttacks()
		self:DoChaseAnimation()
	end)
	 timer.Create( "timer_range_finished_abletorange"..self.Entity:EntIndex(), self.NextRangeAttackTime, 1, function()
...
function ENT:LeapDamageCode()
	...
	if attackthev != nil then
		...
		if GetConVarNumber("vj_npc_dif_normal") == 1 then leapdmg:SetDamage(self.LeapAttackDamage) end -- Normal
		if GetConVarNumber("vj_npc_dif_easy") == 1 then leapdmg:SetDamage(self.LeapAttackDamage/2) end -- Easy
		if GetConVarNumber("vj_npc_dif_hard") == 1 then leapdmg:SetDamage(self.LeapAttackDamage*1.5) end -- Hard
		if GetConVarNumber("vj_npc_dif_hellonearth") == 1 then leapdmg:SetDamage(self.LeapAttackDamage*2.5) end  -- Hell On Earth
	 ...
	 timer.Create( "timer_leap_finished"..self.Entity:EntIndex(), self.NextAnyAttackTime_Leap, 1, function()
		self:StopAttacks()
		self:DoChaseAnimation()
	end)
	 timer.Create( "timer_leap_finished_abletoleap"..self.Entity:EntIndex(), self.NextLeapAttackTime, 1, function()
...
if self.HasMeleeAttackSounds == false then return end
	...
	if randextraattacks == 1 /*&& VJ_PICKRANDOMTABLE(soundtbl) != false*/ then
		if self.ContinuePlayingIdleSoundsOnAttacks == false then VJ_STOPSOUND(self.CurrentIdleSound) end
		if table.Count(self.SoundTbl_MeleeAttackExtra) == 0 then
...
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:RangeAttackSoundCode()
if self.HasSounds == false then return end
...
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:LeapAttackSoundCode()
if self.HasSounds == false then return end
