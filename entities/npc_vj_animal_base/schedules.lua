...
function ENT:RunAI(strExp) -- Called from the engine every 0.1 seconds
	...
	if (!self.CurrentSchedule) && self.VJ_PlayingSequence == false then self:SelectSchedule() end
	if (self.CurrentSchedule) then self:DoSchedule(self.CurrentSchedule) end
	if self.VJ_PlayingSequence == false && self.ShouldBeFlying == false /*&& self:GetSequence() != self.AerialCurrentAnim && self.VJ_IsAerial == false*/ then self:MaintainActivity() end
end
---------------------------------------------------------------------------------------------------------------------------------------------
