...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function VJ_EmitSound(argent,sound,soundlevel,soundpitch)
	if not sound then return end
	if istable(sound) then
		if #sound < 1 then return end -- If the table is empty then end it
		sound = sound[math.random(1,#sound)]
	end
	argent:EmitSound(sound,soundlevel,soundpitch)
...
function VJ_DestroyCombineTurret(vSelf,argent)
	...
	if argent:GetClass() == "npc_turret_floor" && !argent.VJ_TurretDestroyed then
		...
		if phys:IsValid() && phys != nil && phys != NULL then
			if vSelf:IsPlayer() == true then
				phys:EnableMotion(true)
				phys:ApplyForceCenter(vSelf:GetForward() *10000)
			else
				if vSelf:GetEnemy() != nil then
					phys:EnableMotion(true)
					phys:ApplyForceCenter(vSelf:GetForward() *10000)
				end
			end
...
//SetNetworkedBool
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function NPC_MetaTable:VJ_PlaySequence(SequenceID,PlayBackRate,Wait,WaitTime)
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function NPC_MetaTable:VJ_GetNearestPointToEntityDistance(argent)
	if !IsValid(argent) then return end
	local Pos_Enemy, Pos_Self = argent:NearestPoint(self:GetPos() +argent:OBBCenter()), self:NearestPoint(argent:GetPos() +self:OBBCenter())
	Pos_Enemy.z, Pos_Self.z = argent:GetPos().z, self:GetPos().z
	//local Pos_Distance = Pos_Enemy:Distance(Pos_Self)
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function NPC_MetaTable:VJ_ForwardIsHidingZone(StartPos,EndPos)
	...
	nig:Spawn()
	nig:Activate()
	//timer.Simple(3,function() nig:Remove() end) end
	//print(tr.Entity)
	//end*/
	if tr.HitWorld == true && self:GetPos():Distance(tr.HitPos) < 200 then return true end
	if tr.Entity == NULL or tr.Entity:IsNPC() or tr.Entity:IsPlayer() or tr.Entity == self:GetEnemy() or tr.HitWorld == true then
...
local function VJ_NPC_FIREBULLET(Entity,data,Attacker)
	//print(Entity)
	if IsValid(Entity) && !Entity:IsPlayer() && Entity.IsVJBaseSNPC == true && Entity:GetActiveWeapon() != NULL && Entity:GetEnemy() != nil then
		local GetCurrentWeapon = Entity:GetActiveWeapon()
		local EnemyDistance = Entity:GetPos():Distance(Entity:GetEnemy():GetPos())
