...
SWEP.NPC_TimeUntilFire	 		= 0.1 -- How much until the bullet/projectile is fired?
SWEP.NPC_AnimationTbl_General 	= {ACT_RANGE_ATTACK1,ACT_RANGE_ATTACK1_LOW,ACT_IDLE_AGITATED,ACT_IDLE_AIM_AGITATED}
...
SWEP.Secondary.DistantSound		= "vj_ak47/ak47_single_dist.wav" -- The distant sound
	-- General Settings ---------------------------------------------------------------------------------------------------------------------------------------------
SWEP.DryFireSound				= "weapons/vj_dryfire_rifle.wav" -- The sound that it plays when the weapon is out of ammo
	-- Deployment Settings ---------------------------------------------------------------------------------------------------------------------------------------------
...
SWEP.DeploySound				= "weapons/draw_rifle.wav" -- Sound played when the weapon is deployed
	-- Reload Settings ---------------------------------------------------------------------------------------------------------------------------------------------
...
local function FireCode()
	...
	if self:IsValid() && IsValid(self.Owner) && self.Owner:IsValid() && self.Owner:IsNPC() then if self.Owner:GetActivity() != nil && (table.HasValue(self.NPC_AnimationTbl_General,self.Owner:GetActivity()) or table.HasValue(self.NPC_AnimationTbl_Rifle,self.Owner:GetActivity()) or table.HasValue(self.NPC_AnimationTbl_Pistol,self.Owner:GetActivity()) or table.HasValue(self.NPC_AnimationTbl_Shotgun,self.Owner:GetActivity())) then
		FireCode()
	end
 end
end
...
if self.Reloading == true then return end
	...
	self.Weapon:EmitSound(Sound(self.ReloadSound),50,math.random(90,100)) end
	if self.Owner:IsPlayer() then
...
function SWEP:Deploy()
	...
	self.Weapon:EmitSound(Sound(self.DeploySound),50,math.random(90,100)) end
